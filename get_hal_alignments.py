...
#			get_temp = 'head -n ' + str(head_line) + ' temp/' + str(job_id) + '.bed | tail -n500 > temp/' + str(job_id) + '_temp.bed'
			get_temp = 'head -n ' + str(head_line) + ' temp/' + str(job_id) + '.bed > temp/' + str(job_id) + '_temp.bed'
			print get_temp
...
# Is there a zipped output file? Does it contain all the species?
...
# If an unzipped output file, store the results so far
if correct_compressed == 0:
	...
	if os.path.exists(projection_outfile):
		...
		# Store all but the last 500 ids
...
# The id is already in the outpout file
			if id in found_ids:
				found = 1
#				print 'Found ' + id + '!'
			else:
				...
				for target_genome in species:
					...
					# Pull out the lifted over coordinates
...
#			liftover_file = 'temp/liftover/' + str(job_id) + '_' + target_genome + '.bed'
					...
					if check_liftover == 0:
						...
						# Check there is not projections on multiple chromosomes
					...
					# Store the projection per species
...
# Make temporary bed file with human coordinates for one region
...
#	exit()
		...
		# Generates a maf file of the alignment

		hal2maf = '/exports/cmvm/eddie/sbms/groups/young-lab/rob/scripts/hal/hal2maf /exports/cmvm/eddie/sbms/groups/young-lab/rob/genomes/1509_outgroups.hal temp/' + job_id + '.maf --maxRefGap ' + str(length) + ' --refGenome ' + hal_genome + ' --targetGenomes ' + str(','.join(species)) + ' --refTargets temp/' + job_id + '_maf.bed'
...
#	sys.stderr.write(hal2maf + '\n')
	...
	# This block is parsing the maf file
...
#								print fa_species
		...
		# Interpret the fasta output for each species
